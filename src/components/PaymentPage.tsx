...
export default function PaymentPage({ onBack, isAr }: PaymentPageProps) {
    ...
    <div className="min-h-screen pt-32 pb-20 bg-slate-50 px-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium" placeholder="Ex: Mohammed Ahmed" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">{isAr ? "البريد الإلكتروني" : "Email"}</label>
                  <input type="email" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium" placeholder="mail@example.com" />
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input type="text" required maxLength={16} className="w-full p-4 bg-white border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium" placeholder="0000 0000 0000 0000" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" required placeholder="MM/YY" className="w-full p-4 bg-white border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">CVV</label>
                      <input type="text" required maxLength={3} placeholder="123" className="w-full p-4 bg-white border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium" />
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-5">
          ...
          >
            <h3 className="text-xl font-bold text-slate-900 mb-8">{isAr ? "ملخص الطلب" : "Order Summary"}</h3>
            
            <div className="space-y-6">
              <div className="flex justify-between items-center pb-6 border-b border-slate-50 text-right">
                <div>
                  <p className="font-bold text-slate-900">{isAr ? "باقة المشرف الاحترافية" : "Professional Supervisor Plan"}</p>
                  <p className="text-sm text-slate-400">{isAr ? "اشتراك سنوي" : "Annual Subscription"}</p>
                </div>
                <span className="text-lg font-black text-primary">$49.99</span>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-emerald-500 text-sm font-bold">
                  <span>{isAr ? "خصم الافتتاح (20%)" : "Launch Discount (20%)"}</span>
                  <span>-$10.00</span>
                </div>
              </div>
